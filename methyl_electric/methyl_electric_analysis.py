...
def plot_electron_density_projections(electron_density_hist, max_range):
    ...
    plt.tight_layout()
    plt.savefig("plots/methyl_electric_electron.png")
    plt.close("all")
...
def plot_spin_density_projections(spin_density_hist, max_range):
    ...
    plt.tight_layout()
    plt.savefig("plots/methyl_electric_spin.png")
    plt.close("all")


def make_3d_plots(position_data):

    position_data = position_data.reshape(-1, 27)
...
def run():
    ...
    position_data = np.load(
        f"{WORK_DIR}/methyl_electric_inference_12555725/inference_out.npy"
    )[:, :, :, :]
    ...
    position_data = position_data.reshape(-1, 4096, 27)
    ...
    # h3 is the muon position
    h3_position = np.tile(
        np.array(atom_positions["H3"]),
    ...
    plt.savefig(f"plots/methyl_electric_separation_fit")
    plt.close("all")
