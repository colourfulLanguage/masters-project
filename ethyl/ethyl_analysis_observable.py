...
def run():
    ...
    # seperation_hists = np.load(f"{WORK_DIR}/ethyl_inference_11630830_ethyl_muon_spin_correlation.npy")
    # seperation_hists = np.load(f"{WORK_DIR}/ethyl_inference_11810814_ethyl_muon_spin_correlation.npy")
    # seperation_hists = np.load(f"{WORK_DIR}/ethyl_inference_11849300_ethyl_muon_spin_correlation.npy")

    seperation_hists = np.load(
        f"{WORK_DIR}/ethyl_inference_11783960_12823393_100_000/any_muon_spin_correlation.npy"
    )
    ...
    print("Spin Up samples", np.sum(spin_up_bin))
    print("Spin Down samples", np.sum(spin_down_bin))
    print("Spin up electrons", np.sum(spin_up_bin) / (samples))
    print("Spin down electrons", np.sum(spin_down_bin) / (samples))
    ...
    log_y_data_fit_errs = np.nan_to_num(
        y_data_fit_errors / y_data_fit, nan=100000, posinf=100000, neginf=-100000
    )
    ...
    # and the errors for a0
    a0_err = np.sqrt(np.diag(pcov))[0]
    ...
    plt.figure()
    plt.errorbar(
        bin_centers, y_data, yerr=y_err, fmt="o", label="Data with Poisson Errors"
    )
    plt.plot(r_fine, np.exp(y_fitted), "-", label="5th-Order Polynomial Fit")
    plt.xlabel("Separation (bohr)")
    plt.ylabel("Normalized Counts")
    plt.title("Muon-Electron Separation with 5th-Order Polynomial Fit")
    # plot vertical line at horizontal cutoff
    plt.vlines(bin_centers[FIT_CUTOFF], 0, 0.2, colors="r", linestyles="dashed")
    # plt.legend()
    # set y axis between - 0.5 and 0.5
    plt.ylim(0.01, 0.05)
    plt.savefig(f"plots/ethyl_separation_fit_observable.png")
    ...
    A_value_ghz = A_value_freq / 1e9
    print(f"Fermi contact coupling A GHz : {A_value_ghz}")
    ...
    err = (A_value_ghz / y_at_r0) * a0_err
    print("Error", err)
