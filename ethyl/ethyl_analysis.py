...
def plot_muon_density_projections(muon_hist, max_range):
    ...
    plt.tight_layout()
    plt.savefig("plots/ethyl_muon.png")
    plt.close()
...
def plot_electron_density_projections(electron_density_hist, max_range):
    ...
    plt.tight_layout()
    plt.savefig("plots/ethyl_electron.png")
    plt.close()
...
def plot_spin_density_projections(spin_density_hist, max_range):
    ...
    plt.tight_layout()
    plt.savefig("plots/ethyl_spin.png")
    plt.close()
...
def make_3d_plots(spin_up_electron_samples, spin_down_electron_samples, muon_samples):
    ...
    muon_samples = transform(muon_samples.reshape(-1, 3))
...
def run():
    ...
    # position_data = np.load(f"{WORK_DIR}/ethyl_inference_proton_11582048/inference_out.npy")[:25, :, :, :]
    ...
    plt.ylim(-0.1, 0.1)
    plt.legend()
    plt.savefig(f"plots/ethyl_separation_fit")
    plt.close()
