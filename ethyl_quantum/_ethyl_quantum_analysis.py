...
def plot_muon_density_projections(muon_hist, max_range):
    ...
    plt.tight_layout()
    plt.savefig("plots/quantum_ethyl_muon.png")
    plt.close()
...
def plot_electron_density_projections(electron_density_hist, max_range):
    ...
    plt.tight_layout()
    plt.savefig("plots/quantum_ethyl_electron.png")
    plt.close()
...
def plot_spin_density_projections(spin_density_hist, max_range):
    ...
    plt.tight_layout()
    plt.savefig("plots/quantum_ethyl_spin.png")
    plt.close()


def make_3d_plots(spin_up_electron_samples, spin_down_electron_samples, muon_samples):
...
def run():
    ...
    position_data = np.load(
        f"{WORK_DIR}/ethyl_quantum_muon_inference_12564938/inference_out.npy"
    )[:25, :, :, :]
    ...
    make_3d_plots(spin_up_electron_samples, spin_down_electron_samples, muon_samples)

    # h4 is where the "fixed" hydrogen is, but we are in the transformed (rotated) cordinates.
    h4_position = np.tile(
        np.array(transformed_positions["H4"]),
    ...
    plt.ylim(-0.1, 0.1)
    plt.legend()
    plt.savefig(f"plots/quantum_ethyl_separation_fit")
    plt.close()
