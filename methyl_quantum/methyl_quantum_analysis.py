...
def make_3d_plots(
    ...
    spin_up_electron_samples = spin_up_electron_samples - np.tile(
        atom_positions["C"], (len(spin_up_electron_samples), 1)
    )
    spin_down_electron_samples = spin_down_electron_samples - np.tile(
        atom_positions["C"], (len(spin_down_electron_samples), 1)
    )
    proton_samples = proton_samples - np.tile(
        atom_positions["C"], (len(proton_samples), 1)
    )
    muon_samples = muon_samples - np.tile(atom_positions["C"], (len(muon_samples), 1))

    # compute average norm of proton and muon samples
    proton_norm = np.linalg.norm(proton_samples, axis=1)
    muon_norm = np.linalg.norm(muon_samples, axis=1)
    print("Average proton norm", np.mean(proton_norm))
    print("Average muon norm", np.mean(muon_norm))
...
def run():
    ...
    position_data = np.load(
        f"{WORK_DIR}/methyl_quantum_muon_inference_12632287_14370408_2000/inference_out.npy"
    )[:, :, :, :]
    ...
    plt.hlines(-0.014, 0, 0.01, colors="r", linestyles="dashed")
