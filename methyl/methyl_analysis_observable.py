...
import os
import methyl_analysis as methyl_analysis
...
def run():
    ...
    # first_hist = np.load(f"{WORK_DIR}/methyl_inference_10860239_muon_spin_correlation.npy")
    # second_hist = np.load(f"{WORK_DIR}/methyl_inference_10860450_muon_spin_correlation.npy")
    # seperation_hists = first_hist + second_hist

    # seperation_hists = np.load(f"{WORK_DIR}/methyl_inference_10860147_muon_spin_correlation.npy")
    # seperation_hists = np.load(f"{WORK_DIR}/methyl_inference_10860450_muon_spin_correlation.npy")

    # seperation_hists = np.load(f"{WORK_DIR}/methyl_inference_10860239_muon_spin_correlation.npy")
    # seperation_hists = np.load(f"{WORK_DIR}/methyl_inference_andres_10912948_muon_spin_correlation.npy")
    # seperation_hists = np.load(f"{WORK_DIR}/methyl_inference_11780517_muon_spin_correlation.npy")

    seperation_hists = np.load(
        f"{WORK_DIR}/methyl_inference_10864758_12927564_120_000/any_muon_spin_correlation.npy"
    )
    ...
    iterations = (np.sum(seperation_hists)) / (4096 * 9)
    ...
    plt.plot(
        r_fine, -np.exp(y_fitted), "-", label="5th-Order Polynomial Fit", zorder=10
    )
    plt.xlabel("Separation (bohr)")
    plt.ylabel("g(r)")
    plt.title("Methyl unpaired electron-muon correlation function g(r)")
    plt.vlines(
    ...
    plt.ylim(-0.03, 0.03)
    # plt.legend()
    plt.savefig(f"plots/methyl_separation_fit_observable.png")
